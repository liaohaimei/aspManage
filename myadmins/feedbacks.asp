...
Sub main
	...
	sqlStr = replace(replace("select * from {pre}GuestBook "&whereStr&orderStr,"where and","where"),"where order","order")
...
<table width="100%" cellpadding="0" cellspacing="0">
  <tr>
    <td align="left" bgcolor="#6b6b6b" style="color:#FFFFFF; font-weight:bold;text-indent:10px;border-left:solid #e5e5e5 1px;border-bottom:solid #e5e5e5 1px;" valign="middle">留言列表</td>
...
Sub editGuestBook
	...
	sqlStr = "select *  from {pre}GuestBook where ID="&id
	set rsObj = conn.db(sqlStr,"records1")
...
<table width="100%" cellpadding="0" cellspacing="0">
  <tr>
    <td align="left" bgcolor="#6b6b6b" style="color:#FFFFFF; font-weight:bold;text-indent:10px;border-right:solid #e5e5e5 1px; border-left:solid #e5e5e5 1px;border-bottom:solid #e5e5e5 1px;" valign="middle">编辑留言</td>
  </tr>
  <tr>
    <td align="left">
	<table width="100%" cellpadding="0" cellspacing="0" id="RiseList">
        <tbody>
		  ...
		  <tr>
			...
			<table width="80" cellpadding="0" cellspacing="0">
                <tr>
					<td align="right">公 司 名:</td>
                </tr>
            </table></td>
            <td align="left" style="border-top:solid #dddddd 1px;border-right:solid #dddddd 1px;">&nbsp;<%=rsObj("Company")%>&nbsp;</td>
          </tr>
...
Sub saveGuestBook
	...
	select case  actType
		case "edit"
			...
			conn.db  "update {pre}GuestBook set ReplyContent='"&rcontent&"',isshow="&rdate&",ReplyTime='"&now()&"' where id="&m_id,"execute"
...
Sub delGuestBook
	...
	conn.db  "delete from {pre}GuestBook where id="&id,"execute"
...
Sub UpdateOrder
	...
	For i=0 To Ubound(ids)	
		updateSql = "update {pre}GuestBook Set ord="&trim(orders(i))&" Where ID="&trim(ids(i))	
...
Sub delAll
	...
	if ids<>"" Then
	conn.db  "delete from {pre}GuestBook where id in("&ids&")","execute" 
	end if
	parentid = getForm("parentid","get")
