...
Sub echoRunTime()
	endtime=timer()      
	echo "<div align=center><font color=red>页面执行时间: "&FormatNumber((endtime-starttime),4,-1)&"秒 &nbsp;&nbsp;&nbsp;" & conn.queryCount & "次数据查询</font></div>"
End Sub
'获取执行时间
Function getRunTime()
	endtime=timer()
	getRunTime="页面执行时间: "&FormatNumber((endtime-starttime),4,-1)&"秒 &nbsp;" & conn.queryCount & "次数据查询"
End Function
...
Sub selectTypeEdit(typeId,compareValue,parentid)
	...
	loop
		...
		dim names:names=conn.db("select NavName from {pre}Navigation where id="&parentid ,"execute")(0)
